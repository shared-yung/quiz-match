...
**`revealing` の間（公開の進め方）**
1文字目は公開開始の `revealIntervalMs` 後に送り、以降も同じ間隔で1文字ずつ送る。誤答（続行）で `revealing` に戻ったときは、**続きの文字から**1間隔後に再開する。

全文を公開し終えたら `postRevealGraceMs` の猶予に入る。猶予中の押下が誤答（続行）になり `revealing` に戻ったときは、**猶予を最初からやり直す**。残ったプレイヤーに猶予をまるごと与えるため。
...
参加と退室を受け付けるのはホスト。プレイヤーは表示名だけを送り（`join`）、**id はホストが振る**。

- **参加人数は `maxPlayers` まで。** 満員なら断り、`join/rejected` で本人に知らせる。黙って捨てると、正規クライアントの画面が待ったまま止まるため
- **途中参加を許す。** ゲームの途中で入ったプレイヤーは 0 点から始める
- **同じ表示名を許す。** 区別は id でつく。画面で見分けにくい点は UI で扱う
- 退室したプレイヤーは参加者一覧から外れる
- ホストはプレイヤーを兼ねないので、参加者一覧に入らない

## 得点と勝利条件

得点は `scoring` に従って判定のたびに更新する。**時間切れ（無回答）は誤答として扱う**ので `scoring.wrong` が適用される。

勝利条件は問題が `closed` になった時点で評価する。**評価は加点の後**に行い、その判定で入った点を含めて判断する。正解での終了だけでなく、猶予切れや全員ロックアウトで終わったときも評価する。

| `winCondition` | 判定                                                                                                                         |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `firstTo`      | いずれかのプレイヤーが `points` 以上に達したら、その人の勝ち。**達しないまま問題を出し切ったら、その時点の最高得点者が勝ち** |
| `allQuestions` | 用意した問題をすべて消化したら、最高得点者が勝ち                                                                             |

同点の場合は**引き分け**とする。タイブレークは行わない。勝者が複数いれば引き分けで、`game/end` はその全員を勝者として送る。

問題を出し切ったかどうかは、判定の入力として受け取る。何をもって出し切ったとするか（問題リストの終端か、ホストの終了操作か）は、「決めていないこと」の問題文の事前準備と合わせて決める。

**勝利条件を満たしたら、そこでゲームは終わる。** 次の問題へは進まない。
...
- 回答の権利を持っているプレイヤー（`buzzed` / `judging`）の退室と、退室したプレイヤーの記録の扱い（#20 で決める）
- ホストの手動停止。状態表と `question/reveal-stop` にはあるが、状態機械にイベントが無く、停止後に再開できるかも決まっていない（#22 で決める）
